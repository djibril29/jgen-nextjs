...
export function NewsletterFigure({
  ...
  if (resolved) {
    return (
      <div className={cn("relative overflow-hidden bg-jgen-plum", className)}>
        <Image
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden bg-gradient-to-br from-jgen-plum to-[#2d1537]",
        className,
      )}
      // Décoratif : aucune information n'est portée uniquement par ce bloc.
      aria-hidden="true"
    >
      <div className="absolute top-6 right-6 h-20 w-20 rotate-12 bg-jgen-jaune opacity-40" />
      <div className="absolute bottom-8 left-8 h-14 w-14 -rotate-6 bg-jgen-vert opacity-40" />
      <div className="absolute bottom-16 right-16 h-10 w-10 rotate-45 bg-jgen-rose opacity-50" />
